...
const ENTITY_COLLISION_MASK = CollisionFlag.NPC_OCC | CollisionFlag.PLAYER_OCC | CollisionFlag.BLOCK_NPC_AND_PLAYERS;
...
export default class InstanceZone extends Zone {
    ...
    private rotateCollisionFlags(flags: number, rotation: 0 | 1 | 2 | 3): number {
        ...
        const hasWallNorthRoute = !!(flags & CollisionFlag.WALL_NORTH_PROJ_BLOCKER);
        const hasWallEastRoute = !!(flags & CollisionFlag.WALL_EAST_PROJ_BLOCKER);
        const hasWallSouthRoute = !!(flags & CollisionFlag.WALL_SOUTH_PROJ_BLOCKER);
        const hasWallWestRoute = !!(flags & CollisionFlag.WALL_WEST_PROJ_BLOCKER);
        const hasWallNWRoute = !!(flags & CollisionFlag.WALL_NORTH_WEST_PROJ_BLOCKER);
        const hasWallNERoute = !!(flags & CollisionFlag.WALL_NORTH_EAST_PROJ_BLOCKER);
        const hasWallSERoute = !!(flags & CollisionFlag.WALL_SOUTH_EAST_PROJ_BLOCKER);
        const hasWallSWRoute = !!(flags & CollisionFlag.WALL_SOUTH_WEST_PROJ_BLOCKER);
        ...
        if (rotation === 1) {
            ...
            if (hasWallNorthRoute) result |= CollisionFlag.WALL_EAST_PROJ_BLOCKER;
            if (hasWallEastRoute) result |= CollisionFlag.WALL_SOUTH_PROJ_BLOCKER;
            if (hasWallSouthRoute) result |= CollisionFlag.WALL_WEST_PROJ_BLOCKER;
            if (hasWallWestRoute) result |= CollisionFlag.WALL_NORTH_PROJ_BLOCKER;
            if (hasWallNWRoute) result |= CollisionFlag.WALL_NORTH_EAST_PROJ_BLOCKER;
            if (hasWallNERoute) result |= CollisionFlag.WALL_SOUTH_EAST_PROJ_BLOCKER;
            if (hasWallSERoute) result |= CollisionFlag.WALL_SOUTH_WEST_PROJ_BLOCKER;
            if (hasWallSWRoute) result |= CollisionFlag.WALL_NORTH_WEST_PROJ_BLOCKER;
        } else if (rotation === 2) {
            ...
            if (hasWallNorthRoute) result |= CollisionFlag.WALL_SOUTH_PROJ_BLOCKER;
            if (hasWallEastRoute) result |= CollisionFlag.WALL_WEST_PROJ_BLOCKER;
            if (hasWallSouthRoute) result |= CollisionFlag.WALL_NORTH_PROJ_BLOCKER;
            if (hasWallWestRoute) result |= CollisionFlag.WALL_EAST_PROJ_BLOCKER;
            if (hasWallNWRoute) result |= CollisionFlag.WALL_SOUTH_EAST_PROJ_BLOCKER;
            if (hasWallNERoute) result |= CollisionFlag.WALL_SOUTH_WEST_PROJ_BLOCKER;
            if (hasWallSERoute) result |= CollisionFlag.WALL_NORTH_WEST_PROJ_BLOCKER;
            if (hasWallSWRoute) result |= CollisionFlag.WALL_NORTH_EAST_PROJ_BLOCKER;
        } else if (rotation === 3) {
            ...
            if (hasWallNorthRoute) result |= CollisionFlag.WALL_WEST_PROJ_BLOCKER;
            if (hasWallEastRoute) result |= CollisionFlag.WALL_NORTH_PROJ_BLOCKER;
            if (hasWallSouthRoute) result |= CollisionFlag.WALL_EAST_PROJ_BLOCKER;
            if (hasWallWestRoute) result |= CollisionFlag.WALL_SOUTH_PROJ_BLOCKER;
            if (hasWallNWRoute) result |= CollisionFlag.WALL_SOUTH_WEST_PROJ_BLOCKER;
            if (hasWallNERoute) result |= CollisionFlag.WALL_NORTH_WEST_PROJ_BLOCKER;
            if (hasWallSERoute) result |= CollisionFlag.WALL_NORTH_EAST_PROJ_BLOCKER;
            if (hasWallSWRoute) result |= CollisionFlag.WALL_SOUTH_EAST_PROJ_BLOCKER;
        }
