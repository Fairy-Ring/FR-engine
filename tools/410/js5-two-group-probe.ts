...
// One socket: hello, then each [1, archive, 0, 0] request in order; every reply
// must be byte-equal to encodeJs5Group410(archive, 0, store blob).
        ...
        const pump = () => {
            while (verified < sent && total >= reqs[verified][1].length) {
                ...
                if (!buf.subarray(0, want.length).equals(want)) {
                    finish(`group ${verified} bytes differ from store encode`);
                    return;
                }
            ...
            if (sent === verified) {
                s.write(Buffer.from([1, reqs[verified][0], 0, 0]));
                sent += 1;
            }
        };
        s.setTimeout(10000);
...
const blob255 = store.read(255, 0);
if (blob255 === null) {
...
const err = await fetchGroups(host, port, [
    [255, encodeJs5Group410(255, 0, blob255)],
    [5, encodeJs5Group410(5, 0, blob5)]
]);
if (err !== null) {
    console.error(`FAIL ${err}`);
    process.exit(1);
}
console.log('PASS js5 255/0 then 5/0 same socket');
