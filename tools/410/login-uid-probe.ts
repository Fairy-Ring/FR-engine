...
if (inner.kind !== 'ok') {
    ...
    if (inner.uid !== 0) {
        fails.push(`uid=${inner.uid} want 0`);
    }
    if (inner.username !== 0n) {
        fails.push(`username=${inner.username} want 0`);
    }
    if (inner.password !== '') {
        fails.push(`password=${JSON.stringify(inner.password)} want ''`);
    }
}
...
console.log('PASS login uid=0 name=0 pw-empty');
